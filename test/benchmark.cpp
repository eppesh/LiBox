    ...
    while (getline(fin, line)) {
        ...
        try {
            ...
            if (start_key <= end_key) {
                ...
                cerr << "Warning: Invalid range [" << start_key << ", " << end_key
                     << "], start > end" << endl;
            }
    ...
    while (getline(fin, line)) {
        ...
        try {
            ...
            if (scan_count > 0) {
                ...
                cerr << "Warning: Invalid scan count " << scan_count << " for key " << start_key
                     << endl;
            }
        } catch (const exception& e) {
...
int main(int argc, char* argv[]) {
    ...
    liboxns::LiBox<KeyType, ValueType> index(init_underflow,
                                             init_overflow,
                                             &splitTaskQueue,
                                             thread_num);
...
#pragma omp parallel for reduction(+ : batch_found)
            ...
            batch_lookup_time = chrono::duration_cast<chrono::nanoseconds>(lookups_end_time -
                                                                           lookups_start_time)
                                    .count();
...
#pragma omp parallel for reduction(+ : inserted)
            for (int j = i; j < num_keys_after_batch; j++) {
                if (index.insertKeyValue(file_keys[j], 1).status ==
                    liboxns::InsertStatus::SUCCESS) {
                    inserted++;
                }
            }
            ...
            batch_insert_time = chrono::duration_cast<chrono::nanoseconds>(inserts_end_time -
                                                                           inserts_start_time)
                                    .count();
        ...
        if (print_batch_stats) {
            ...
            cout << "Batch " << batch_no << ", cumulative ops: " << cumulative_operations
                 << "\n\tbatch throughput:\t" << batch_lookup_mops << " Mop/s (lookups),\t"
                 << batch_insert_mops << " Mop/s (inserts),\t" << batch_total_mops
                 << " Mop/s (total)"
                 << "\n\tcumulative throughput:\t" << cumulative_lookup_mops
                 << " Mop/s (lookups),\t" << cumulative_insert_mops << " Mop/s (inserts),\t"
                 << cumulative_total_mops << " Mop/s (total)" << endl;
        }
    ...
    if (enable_range_search && !range_queries.empty()) {
        ...
        cout << "Testing " << range_queries.size() << " range queries, " << range_search_repeat
             << " times each..." << endl;
...
#pragma omp parallel for reduction(+ : total_results_found)
        ...
        double total_range_time =
            chrono::duration_cast<chrono::nanoseconds>(range_end_time - range_start_time).count();
        ...
        cout << "  Average results per query: " << fixed << setprecision(2) << avg_results_per_query
             << endl;
        cout << "  Throughput: " << fixed << setprecision(3) << range_search_mops << " Mop/s"
             << endl;
        cout << "  Average time per query: " << fixed << setprecision(3) << avg_time_per_query_us
             << " μs" << endl;
        ...
        if (validate_range_results_flag) {
            ...
            if (validation_errors == 0) {
                cout << "  ✓ All " << range_queries.size() << " range queries passed validation!"
                     << endl;
                cout << "  ✓ Total " << validation_total_results
                     << " results are all within correct ranges." << endl;
            } else {
        ...
        for (size_t q = 0; q < min(size_t(5), range_queries.size()); q++) {
            const auto& results = all_results[q];
            cout << "  Query " << q << " [" << range_queries[q].first << ", "
                 << range_queries[q].second << "]: " << results.size() << " results";
            if (!results.empty()) {
    ...
    if (enable_scan_mode) {
        ...
        while (true) {
            ...
            if (num_scans_per_batch > 0 && !scan_queries.empty()) {
                ...
                sort(batch_scan_queries.begin(),
                     batch_scan_queries.end(),
...
#pragma omp for
                    for (int j = 0; j < num_scans_per_batch; j++) {
                        ...
                        size_t results_count = index.scan_optimized(start_key,
                                                                    scan_count,
                                                                    thread_scan_buffer);
                ...
                batch_scan_time = chrono::duration_cast<chrono::nanoseconds>(scan_end_time -
                                                                             scan_start_time)
                                      .count();
...
#pragma omp parallel for reduction(+ : inserted)
            ...
            if (print_batch_stats) {
                ...
                double batch_scan_mops = (batch_scan_time > 0)
                                             ? num_scans_per_batch / batch_scan_time * 1000
                                             : 0;
                double batch_insert_mops = (batch_insert_time > 0)
                                               ? num_actual_inserts / batch_insert_time * 1000
                                               : 0;
                double batch_total_mops = (batch_time > 0)
                                              ? num_batch_operations / batch_time * 1000
                                              : 0;

                double cumulative_scan_mops = (cumulative_scan_time > 0)
                                                  ? cumulative_scans / cumulative_scan_time * 1000
                                                  : 0;
                double cumulative_insert_mops = (cumulative_insert_time > 0)
                                                    ? cumulative_inserts / cumulative_insert_time *
                                                          1000
                                                    : 0;
                double cumulative_total_mops = (cumulative_time > 0)
                                                   ? cumulative_operations / cumulative_time * 1000
                                                   : 0;

                double avg_results_per_scan = (cumulative_scans > 0)
                                                  ? (double)cumulative_scan_results /
                                                        cumulative_scans
                                                  : 0;
                ...
                cout << "Optimized Batch " << batch_no << ", ops: " << cumulative_operations
                     ...
                     << ")"
                     << "\n\tbatch throughput:\t" << fixed << setprecision(3) << batch_scan_mops
                     << " Mop/s (scans, "
                     << (num_scans_per_batch > 0 ? (double)batch_scan_results / num_scans_per_batch
                                                 : 0)
                     << " avg results),\t" << batch_insert_mops << " Mop/s (inserts),\t"
                     ...
                     << cumulative_insert_mops << " Mop/s (inserts),\t" << cumulative_total_mops
                     << " Mop/s (total)" << endl;
            }
            ...
            for (int i = 0; i < validation_samples; i++) {
                ...
                if (!validate_scan_results(validation_buffer, start_key, scan_count)) {
                    ...
                    cout << "  ERROR in scan query " << i << " (start_key=" << start_key
                         << ", count=" << scan_count << ", actual_results=" << results_count << ")"
                         << endl;
                }
            }
        ...
        cout << "\nCumulative stats: " << batch_no << " batches, " << cumulative_operations
             << " ops (" << cumulative_lookups << " lookups, " << cumulative_inserts << " inserts)"
    ...
    if (cumulative_lookups > 0) {
        cout << "Found " << cumulative_found << " matches out of " << cumulative_lookups
             << " lookups (" << (static_cast<double>(cumulative_found) / cumulative_lookups * 100.0)
             << "%)" << endl;
    }
