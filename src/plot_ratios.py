...
def plot_ratios(df, output_file=None, show_plot=True):
    ...
    for i, window_size in enumerate(window_sizes):
        window_data = df[df['window_size'] == window_size].sort_values('segment_length')
        
        plt.plot(window_data['segment_length'], 
...
def plot_separate_ratios(df, output_file=None, show_plot=True):
    ...
    for i, window_size in enumerate(window_sizes):
        window_data = df[df['window_size'] == window_size].sort_values('segment_length')
        ax1.plot(window_data['segment_length'], 
                window_data['underflow_ratio'], 
                color=colors[i], 
                linewidth=2,
    ...
    ax1.set_title('Underflow Ratios by Segment Length', fontsize=14)
    ax1.grid(True, alpha=0.3)
    ax1.legend(fontsize=10)
    
    for i, window_size in enumerate(window_sizes):
        window_data = df[df['window_size'] == window_size].sort_values('segment_length')
        ax2.plot(window_data['segment_length'], 
                window_data['overflow_ratio'], 
                color=colors[i], 
                linewidth=2,
                label=f'Window: {window_size:,}')
    ...
    ax2.set_title('Overflow Ratios by Segment Length', fontsize=14)
