...
bool load_data(const std::string &input, std::vector<KeyType>& data){
    ...
    } else if (input.find ("umass") != std::string::npos || input.find (".csv") != std::string::npos ||
...
int main(int argc, char* argv[]){
    ...
    if(argc < 3){
        ...
        std::cerr << "E.g.: ./partition_optimization w106.csv optimized_segments_w106.csv" << std::endl;
        return -1;
    }
    ...
    std::cout << "data size=" << data.size() << ", min=" << data.front() << ", max=" << data.back() << std::endl;
    vector<Block<KeyType>> blocks = computeBlocks(data, BLOCK_SIZE);
    std::cout << "Compute blocks finished: block count=" << blocks.size() << "; lower=" << blocks.back().startKey 
        << ", upper=" << blocks.back().endKey << ", range=" << blocks.back().range << std::endl;

    int maxMergeCount = 3;
    double underflowThreshold = 0.5;
    double overflowThreshold = 0.1;
    std::cout << "maxMergeCount=" << maxMergeCount << ", ufthreshold=" << underflowThreshold << ", ofthreshold=" << overflowThreshold << std::endl;
    vector<StructSegment<KeyType>> initSegments = partitionSegmentsOverall(blocks, data, underflowThreshold, maxMergeCount);
    std::cout << "initSegments finished! init size=" << initSegments.size() << std::endl;
    std::cout << "last segment: lower=" << initSegments.back().seg_lower << "; upper=" << initSegments.back().seg_upper << "; range=" << initSegments.back().box_range << std::endl;
    vector<StructSegment<KeyType>> finalSegments = expandSegments(initSegments, blocks, data, underflowThreshold, overflowThreshold, maxMergeCount);
    std::cout << "finalSegments finished! final size=" << finalSegments.size() << std::endl;
    std::cout << "last segment: lower=" << finalSegments.back().seg_lower << "; upper=" << finalSegments.back().seg_upper << "; range=" << finalSegments.back().box_range << std::endl;
