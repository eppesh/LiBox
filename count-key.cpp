...
public:
    ...
    void read_keys_from_file(const std::string& filename) {
        ...
        std::cout << "Reading keys from " << filename << "..." << std::endl;
        ...
        std::cout << "\rCompleted: " << line_count << " keys read in " 
                  ...
                  << static_cast<int>(rate) << " keys/sec)" << std::endl;
    }
...
int main(int argc, char* argv[]) {
    if (argc < 5) {
        std::cerr << "Usage: " << argv[0] << " <csv_file> <min_key> <max_key> <num_buckets> [output_file]" << std::endl;
    ...
    counter.read_keys_from_file(input_file);
